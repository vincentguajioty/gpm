...
if (centreCoutsEstCharge($_SESSION['idPersonne'],$_GET['idCentreDeCout'])==0)
{
    ...
    $query = $db->prepare('INSERT INTO DOCUMENTS_CENTRE_COUTS(idCentreDeCout, nomDocCouts, formatDocCouts, dateDocCouts, urlFichierDocCouts, idTypeDocument) VALUES(:idCentreDeCout, :nomDocCouts, :formatDocCouts, CURRENT_TIMESTAMP, :urlFichierDocCouts, :idTypeDocument);');
    $query->execute(array(
    ...
    if ($resultat) {
        ...
        writeInLogs("Ajout d'une pièce jointe référence " . $_POST['nomDocCouts'] . " au centre de cout " . $_GET['idCentreDeCout'], '2');
    } else {
        $query = $db->prepare('DELETE FROM DOCUMENTS_CENTRE_COUTS WHERE idDocCouts = :idDocCouts;');
        $query->execute(array(
            'idDocCouts' => $data['idDocCouts']
        ));
        writeInLogs("Erreur inconnue lors de l'ajout d'une pièce jointe au centre de couts " . $_GET['idCentreDeCout'], '5');
