...
const OneCentreCommandesAIntegrer = ({
    idCentreDeCout,
    commandes,
    setPageNeedsRefresh,
    tabInReadOnly,
                ...
                if(!tabInReadOnly)
                {
                    return(<>
                        <IconButton
                            ...
                            icon='check'
                            disabled={isLoading}
                            onClick={()=>{integrerCmd(row.original.idCommande)}}
                        >Intégrer la commande</IconButton>
                        <br/>
                        <IconButton
                            className='me-1 mb-1'
                            size='sm'
                            ...
                            icon='ban'
                            disabled={isLoading}
                            onClick={()=>{refuserCmd(row.original.idCommande)}}
                        >Refuser la commande</IconButton>
                    </>);
                }
			},
        },
    ...
    const integrerCmd = async (idCommande) => {
        try {
            setLoading(true);
            
            await Axios.post('/centresCouts/integrerCommande',{
    ...
    const refuserCmd = async (idCommande) => {
        try {
            setLoading(true);
            
            await Axios.post('/centresCouts/refuserCommande',{
                idCommande: idCommande,
                idCentreDeCout: idCentreDeCout,
            });
...
OneCentreCommandesAIntegrer.propTypes = {};

export default OneCentreCommandesAIntegrer;