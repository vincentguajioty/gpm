...
else
{
    ...
    $query = $db->prepare('UPDATE CENTRE_COUTS_OPERATIONS SET
        ...
        WHERE idOperations = :idOperations;');
    $query->execute(array(
    ...
    {
        case '00000':
            writeInLogs("Modification d'une opération dans le centre de couts " . $_GET['idCentreDeCout'], '3');
            $_SESSION['returnMessage'] = 'Opération modifiée avec succès.';
            $_SESSION['returnType'] = '1';
            break;

        default:
            writeInLogs("Erreur inconnue lors de la modification d'une opération dans le centre de couts " . $_GET['idCentreDeCout'], '5');
