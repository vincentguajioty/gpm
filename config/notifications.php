...
if ($nbDest > 0)
{
    ...
    $nbAlertes = $nbManquant + $nbPerime + $nbLotsNOK + $nbManquantReserve + $nbPerimeReserve + $nbAssurance + $nbRevisions + $nbInventaires + $nbInventairesReserve;
    ...
    $message_html = "Bonjour, <br/><br/>Ceci est une notification journalière d'alerte sur " . $APPNAME . "<br/><br/>";
    
    if ($nbPerime > 0)
	{
	    $message_html = $message_html . "Alertes de péremption des consommables dans les lots:<br/><ul>";
	...
	if ($nbInventaires > 0)
	{
	    $message_html = $message_html . "Alertes d'inventaires des lots:<br/><ul>";
	...
	{
	    $message_html = $message_html . "Alertes d'inventaires des conteneurs de réserve:<br/><ul>";
	    ...
	    while($data = $query->fetch())
	    {
	        $message_html = $message_html . "<li>".$data['libelleConteneur'] . "</li>";
	    }
	    $message_html = $message_html."</ul><br/><br/>";
	}
	...
	if ($nbAlertes == 0)
	{
	    $message_html = $message_html . "Aucune alerte sur le parc.<br/><br/>";
	}
